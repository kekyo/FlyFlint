...
namespace FlyFlint.Nullability.Cecil
{
    /// <summary>
    ...
    internal sealed class NullabilityInfoContext
    {
        ...
        private NullabilityState? GetNullableContext(MemberInfo? memberInfo)
        {
            while (memberInfo != null)
            {
                ...
                {
                    if (attribute.GetAttributeType().Name == "NullableContextAttribute" &&
                        attribute.GetAttributeType().Namespace == CompilerServicesNameSpace &&
        ...
        private void CheckNullabilityAttributes(NullabilityInfo nullability, IList<CustomAttributeData> attributes)
        {
            ...
            {
                if (attribute.GetAttributeType().Namespace == "System.Diagnostics.CodeAnalysis")
                {
                    if (attribute.GetAttributeType().Name == "NotNullAttribute")
                    {
                        codeAnalysisReadState = NullabilityState.NotNull;
                    }
                    else if ((attribute.GetAttributeType().Name == "MaybeNullAttribute" ||
                            attribute.GetAttributeType().Name == "MaybeNullWhenAttribute") &&
                    ...
                    else if (attribute.GetAttributeType().Name == "DisallowNullAttribute")
                    {
                        codeAnalysisWriteState = NullabilityState.NotNull;
                    }
                    else if (attribute.GetAttributeType().Name == "AllowNullAttribute" &&
        ...
        private NotAnnotatedStatus PopulateAnnotationInfo(IList<CustomAttributeData> customAttributes)
        {
            ...
            {
                if (attribute.GetAttributeType().Name == "NullablePublicOnlyAttribute" &&
                    attribute.GetAttributeType().Namespace == CompilerServicesNameSpace &&
                    attribute.ConstructorArguments.Count == 1)
                {
        ...
        private NullabilityInfo GetNullabilityInfo(MemberInfo memberInfo, Type type, NullableAttributeStateParser parser, ref int index)
        {
            ...
            NullabilityInfo[] genericArgumentsState = new NullabilityInfo[0];
        ...
        private static NullableAttributeStateParser CreateParser(IList<CustomAttributeData> customAttributes)
        {
            foreach (CustomAttributeData attribute in customAttributes)
            {
                if (attribute.GetAttributeType().Name == "NullableAttribute" &&
                    attribute.GetAttributeType().Namespace == CompilerServicesNameSpace &&
                    attribute.ConstructorArguments.Count == 1)
                {
